...
public class BurgerVariables {
	public static class MapVariables extends WorldSavedData {
		public static final String DATA_NAME = "burger_mapvars";
		public MapVariables() {
			super(DATA_NAME);
		}
		...
		public void syncData(World world) {
			this.markDirty();
			if (world.isRemote) {
				Burger.PACKET_HANDLER.sendToServer(new WorldSavedDataSyncMessage(0, this));
			} else {
				Burger.PACKET_HANDLER.sendToAll(new WorldSavedDataSyncMessage(0, this));
			}
		}
	...
	public static class WorldVariables extends WorldSavedData {
		public static final String DATA_NAME = "burger_worldvars";
		...
		public void syncData(World world) {
			this.markDirty();
			if (world.isRemote) {
				Burger.PACKET_HANDLER.sendToServer(new WorldSavedDataSyncMessage(1, this));
			} else {
				Burger.PACKET_HANDLER.sendToDimension(new WorldSavedDataSyncMessage(1, this), world.provider.getDimension());
			}
		}
	...
	public static class WorldSavedDataSyncMessageHandler implements IMessageHandler<WorldSavedDataSyncMessage, IMessage> {
		...
		private void syncData(WorldSavedDataSyncMessage message, MessageContext context, World world) {
			if (context.side == Side.SERVER) {
				...
				if (message.type == 0)
					Burger.PACKET_HANDLER.sendToAll(message);
				else
					Burger.PACKET_HANDLER.sendToDimension(message, world.provider.getDimension());
			}
			if (message.type == 0) {
